...
<%@ taglib prefix="common" uri="http://itcast.cn/common/"%>
<!-- 
	basePath:代表的含义和${pageContext.request.contextPath} 差不多
	basePath的值等于：http://localhost:8080/crm_ssm/
	path:获取的是项目名  /crm_ssm
					...
					<form class="form-inline" action="${pageContext.request.contextPath }/customer/list.action" method="get">
						...
						<div class="form-group">
							...
							<select	class="form-control" id="customerFrom" placeholder="客户来源" name="custSource">
								...
								<c:forEach items="${listSource}" var="basedict">
									<option value="${basedict.dict_id}" <c:if test="${custSource==basedict.dict_id}">selected</c:if> >
									   ${basedict.dict_item_name}
									</option>
								</c:forEach>
							</select>
						</div>
						<div class="form-group">
							...
							<select	class="form-control" id="custIndustry"  name="custIndustry">
								<option value="">--请选择--</option>
								<c:forEach items="${listIndustry}" var="basedict">
									<option value="${basedict.dict_id}" <c:if test="${custIndustry==basedict.dict_id}">selected</c:if> > 
										${basedict.dict_item_name}
									</option>
								</c:forEach>
							</select>
						...
						<div class="form-group">
							...
							<select	class="form-control" id="custLevel" name="custLevel">
								<option value="">--请选择--</option>
								<c:forEach items="${listLevel}" var="basedict">
									<option value="${basedict.dict_id}"  <c:if test="${custLevel==basedict.dict_id}">selected</c:if> >
										${basedict.dict_item_name}
									</option>
								</c:forEach>
							</select>
				...
				<div class="col-lg-12">
					<div class="panel panel-default">
						...
						<div class="col-md-12 text-right">
						  <common:page url="${pageContext.request.contextPath }/customer/list.action" /> 
						</div>
						<!-- /.panel-body -->
					</div>
					<!-- /.panel -->
